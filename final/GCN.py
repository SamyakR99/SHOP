...
class DualCoop(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        if cfg.DATASET.NAME == 'unimib':
            self.relation = torch.load('/home/samyakr2/SHOP/ARK/co_occurrence_matrix_unimib.pth')
            ...
            self.relation = torch.load('/home/samyakr2/SHOP/ARK/co_occurrence_matrix_food103.pth') #torch.Tensor(np.load(realtion_path)).to(torch.float32)
            ...
            self.relation = torch.load('/home/samyakr2/SHOP/ARK/co_occurrence_matrix_voc2007.pth') #torch.Tensor(np.load(realtion_path)).to(torch.float32)
    ...
    def forward(self, image, cls_id=None):
        ...
        b, c = output.shape
        # resized_output = output.resize(b, 2, c//2)
        resized_output_neg = output[:,:c//2]#.unsqueeze(-1) ## 32,103 
        resized_output_pos = output[:,c//2:]#.unsqueeze(-1)
        ...
        # if torch.any(torch.isnan(self.gcn_relation)):
        #     print("gcn_relation contains NaN values")
        #     breakpoint()   
        tf1 = self.gc1(tf1, self.gcn_relation.cuda())
        # if torch.any(torch.isnan(tf1)):
        #     print("tf1 contains NaN values")
        #     breakpoint()
        tf1 = self.relu(tf1)

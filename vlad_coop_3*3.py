...
class NetVLAD(nn.Module):
    def __init__(self, num_clusters, feature_size):
        ...
        self.centroids = nn.Parameter(torch.randn(num_clusters, feature_size))
        self.conv = nn.Conv2d(feature_size, num_clusters, kernel_size=(3, 3), padding =1 ,bias=True)
...
class AttentionConv(nn.Module):
    def __init__(self, embed_dim: int, spec_dim: int,  output_dim: int = None):
        ...
        self.v_proj = nn.Linear(embed_dim, embed_dim)
        self.c_proj = nn.Linear(embed_dim, output_dim or embed_dim)
        self.linear_projection = nn.Linear(2048, 512)
        
    def forward(self, x):
#         print("X", x.shape) #32, 2048, 14, 14
        N, C, h, w = x.shape
        ...
        # Instantiate the NetVLAD module
        netvlad = NetVLAD(num_clusters, feature_size)
...
#         print('X_POS', x.shape) #197, 32, 2048
        with torch.no_grad():
...
#         print('self.c_proj.weight, self.c_proj.bias',self.c_proj.weight.shape, self.c_proj.bias.shape)
#         x2 = F.linear(out_vlad, self.c_proj.weight, self.c_proj.bias)
